...
public class ExamInspectReportListServiceImpl extends ServiceImpl<ExamInspectReportListMapper, ExamInspectReportList> implements IExamInspectReportListService {
    ...
    @Override
    public void saveBatchInspectReportList(List<ExamInspectReportList> inspectReportLists) {
        if(CollectionUtils.isEmpty(inspectReportLists)){
            return;
        }
//        ExecutorService pool = Executors.newFixedThreadPool(Constant.THREAD_COUNT);
//        inspectReportLists.forEach (report -> {
//            pool.submit(new Runnable() {
//                @Override
//                public void run() {
//                    saveInspectReportList(report);
//                }
//            });
//        });
//        pool.shutdown();
